...
def test_update_overwrites_skill_keeps_operator_edits(tmp_path: Path) -> None:
    ...
    init_studio(studio)
    skill = studio / ".cursor" / "skills" / "prime-runner" / "SKILL.md"
    ...
    support = studio / ".cursor" / "skills" / "ui-direction" / "SKILL.md"
...
def test_doctor_missing_skill(tmp_path: Path) -> None:
    studio = tmp_path / "studio"
    init_studio(studio)
    (studio / ".cursor" / "skills" / "prime-verify" / "SKILL.md").unlink()
    result = doctor_studio(studio)
    assert result.ok is False
...
def test_doctor_missing_support_skill(tmp_path: Path) -> None:
    studio = tmp_path / "studio"
    init_studio(studio)
    skill = studio / ".cursor" / "skills" / "ux-architect" / "SKILL.md"
    skill.unlink()
    result = doctor_studio(studio)
    assert result.ok is False
